...
namespace Ioc
{
	...
	public:
		...
		TEST_METHOD(PolymorphicResolve)
		{
			...
			Assert::AreEqual(50, pIoc->Resolve<Base>()->Test());
		}
		...
		TEST_METHOD(ParameterizedResolve)
		{
			...
			Assert::AreEqual("test"s, pIoc->Resolve<ParameterizedClass>({ .s = "test"s })->s);
		}
		...
		TEST_METHOD(CascadedResolve)
		{
			...
			Assert::AreEqual(100, pIoc->Resolve<Dependant>()->pDependency->Test());
		}
		...
		TEST_METHOD(IndependentResolve)
		{
			...
			pFirst->s = "test"s;
			Assert::AreEqual("test"s, pFirst->s);
			Assert::AreNotEqual("test"s, pSecond->s);
		}
		...
		TEST_METHOD(ReplacementInjection)
		{
			...
			Assert::AreEqual(50, pIoc->Resolve<Dependant>()->pDependency->Test());
		}
	private:
